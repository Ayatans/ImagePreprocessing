...
sys.path.append('/data3/yczhang/code/DCNet/')
from maskrcnn_benchmark.data.datasets.dior import DIORDataset
from collections import OrderedDict

# split = int(sys.argv[1])
for split in range(1, 6):
    dataset = DIORDataset('/data3/yczhang/dataset/DIOR-DCNet', 'trainval')
    keeps = []
    ...
    with open(
            '/data3/yczhang/dataset/DIOR-DCNet/ImageSets/Main/trainval_split%d_base.txt' % (split),
            'w+') as f:
        for i in keeps:
    ...
    dataset = DIORDataset('/data3/yczhang/dataset/DIOR-DCNet', 'test', use_difficult=True)
    keeps = []
    all_cls = DIORDataset.CLASSES
    novel_cls = [DIORDataset.CLASSES_SPLIT1_NOVEL,
                 DIORDataset.CLASSES_SPLIT2_NOVEL,
    ...
    print(dict(zip(all_cls, box_count)))
    with open('/data3/yczhang/dataset/DIOR-DCNet/ImageSets/Main/test_split%d_base.txt' % (split),
              'w+') as f:
        for i in keeps:
            f.write(dataset.ids[i] + '\n')
