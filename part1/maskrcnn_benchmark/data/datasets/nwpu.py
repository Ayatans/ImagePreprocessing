...
normalize_opt = Normalize(mean=[102.9801, 115.9465, 122.7717], std=[1., 1., 1.], to_bgr255=True)  # 图像减均值除方差参数
...
class NWPUDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, split, use_difficult=False, transforms=None, toofew=True, seed=0):
        ...
        if 'split1_base' in split:
            cls = NWPUDataset.CLASSES_SPLIT1_BASE
        else:
            cls = NWPUDataset.CLASSES
        self.cls = cls
    ...
    def get_groundtruth(self, index):
        ...
        # target.add_field("difficult", anno["difficult"])  # dior没有difficult注释
        return target
    ...
    def _preprocess_annotation(self, target):
        boxes = []
        gt_classes = []
        difficult_boxes = []  # 这里如果后面对返回的res报错，就把diff boxes全置位0
        TO_REMOVE = 1

        thisname = target.find("filename").text  # 当前image id
        for obj in target.iter("object"):
            # difficult = int(obj.find("difficult").text) == 1
            # if not self.keep_difficult and difficult:
            #     continue
            # name = obj.find("name").text.lower().strip()  # 原代码加了lower
