...
class NWPUDataset_Meta(torch.utils.data.Dataset):
    CLASSES = ("__background__ ", 'airplane', 'baseballdiamond', "basketballcourt", "bridge", "groundtrackfield",
               "harbor", "ship", "storagetank", 'tenniscourt', "vehicle")
    #
    CLASSES_SPLIT1_BASE = ("__background__ ", "basketballcourt", "bridge", "groundtrackfield",
                           "harbor", "ship", "storagetank", "vehicle")

    CLASSES_SPLIT1_NOVEL = ('airplane', 'baseballdiamond', 'tenniscourt')
    #
    # CLASSES_SPLIT1_BASE = ("__background__ ", 'airplane', 'baseballdiamond', "bridge",
    #                        "harbor", "storagetank", 'tenniscourt',"vehicle")
    #
    # CLASSES_SPLIT1_NOVEL = ( "basketballcourt", "groundtrackfield", "ship")
    ...
    def __init__(self, data_dir, split, use_difficult=False, transforms=None, toofew=True, shots=200, size=224, seed=0):
        ...
        if 'split1_base' in split:
            cls = NWPUDataset_Meta.CLASSES_SPLIT1_BASE
        else:
            phase = 2
            cls = NWPUDataset_Meta.CLASSES
        self.cls = cls[1:]
        self.class_to_ind = dict(zip(cls, range(len(cls))))
        self.categories = dict(zip(range(len(cls)), cls))
        self.list_root = os.path.join('/remote-home/yczhang/code/DCNet', 'fs_list')
        if 'base' in split:
            fname = 'nwpu_traindict_full.txt'
            metafile = os.path.join(self.list_root, fname)
        else:
            fname = 'nwpu_traindict_bbox_' + str(shots) + 'shot.txt'
            metafile = os.path.join(self.list_root, fname)
        if 'standard' in split and seed > 0:
            fname = 'nwpu_traindict_bbox_' + str(shots) + 'shot_seed' + str(seed) + '.txt'
            metafile = os.path.join(self.list_root, fname)
        metainds = [[]] * len(self.cls)
        # 去nwpulist中找class_trainval.txt
        with open(metafile, 'r') as f:
            metafiles = []
            for line in f.readlines():
                pair = line.rstrip().split()    # 一般都是 class 路径，所以len=2
                if len(pair) == 2:
                    pass
                elif len(pair) == 4:
            ...
            metafiles = {k: v for k, v in metafiles}    # class: path
            self.metalines = [[]] * len(self.cls)
            # 遍历类别
            for i, clsname in enumerate(self.cls):
                with open(metafiles[clsname], 'r') as imgf:     # 打开的是diorlist1中不同类别在不同shot下用到的训练图片id or airplane_trainval.txt
                    lines = [l for l in imgf.readlines()]
                    self.metalines[i] = lines
                    if (shots > 15):   # metalines包含了该类所有训练图像，从该类的图片id中随机抽shots个。shots<100就全用？
    ...
    def __getitem__(self, index):
        ...
        for cls_id, img_id in img_ids:
            ...
            for obj in target.iter('object'):  # 遍历个某张图内所有的<object>元素。也就是1个标签
                ...
                # if len(self.chongfu)>400:
                #     self.chongfu=[]
                # if [cls_id, img_id, name, bndbox] in self.chongfu:
                #     continue
                # else:
                #     self.chongfu.append([cls_id, img_id, name, bndbox])
                #     break
                break
        ...
        res = torch.cat(data, dim=0)    # torch.FloatTensor  [7, 4, 256, 256] 第0维为类数
        return res
