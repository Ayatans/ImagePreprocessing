...
for split in range(1, 4):
    for year in ["07", "12"]:
        dataset = PascalVOCDataset('/root/code/zyc/dataset/VOC/VOCdevkit/VOC20%s'%year, 'trainval')
        keeps = []
        ...
        with open('/root/code/zyc/dataset/VOC/VOCdevkit/VOC20%s/ImageSets/Main/trainval_split%d_base.txt'%(year, split), 'w+') as f:
            for i in keeps:
                f.write(dataset.ids[i] + '\n')

        if year == '12':
            continue
        dataset = PascalVOCDataset('/root/code/zyc/dataset/VOC/VOCdevkit/VOC20%s'%year, 'test', use_difficult=True)
        keeps = []
        all_cls = PascalVOCDataset.CLASSES
        novel_cls = [PascalVOCDataset.CLASSES_SPLIT1_NOVEL,
                     PascalVOCDataset.CLASSES_SPLIT2_NOVEL,
        ...
        print(dict(zip(all_cls, box_count)))
        with open('/root/code/zyc/dataset/VOC/VOCdevkit/VOC20%s/ImageSets/Main/test_split%d_base.txt'%(year, split), 'w+') as f:
            for i in keeps:
                f.write(dataset.ids[i] + '\n')
