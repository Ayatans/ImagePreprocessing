...
class FPNPredictor(nn.Module):
    def __init__(self, cfg, in_channels):   # 1024
        super(FPNPredictor, self).__init__()
        num_classes = cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        representation_size = in_channels   # 1024

        self.cls_score = nn.Linear(representation_size, num_classes)    # 1024， 21
        self.num_bbox_reg_classes = 2 if cfg.MODEL.CLS_AGNOSTIC_BBOX_REG else num_classes   # num_classes
        #self.bbox_pred = nn.Linear(representation_size, num_bbox_reg_classes * 4)
        self.bbox_pred = nn.Linear(representation_size, 4)  # 1024, 4
    ...
    def forward(self, xc, xr=None):
        # 只进if
        if xr is not None:
            scores = self.cls_score(xc) # 分类得分要用relu后的 256*bs/GPU * 1024 -- 1024*21 = 256*bs/GPU * 21 shape
            # box回归要用relu前的, 和FSCE最后一维不同似乎因为num_bbox_reg_classes 4*21=84维
...
def make_roi_box_predictor(cfg, in_channels):
    func = registry.ROI_BOX_PREDICTOR[cfg.MODEL.ROI_BOX_HEAD.PREDICTOR] # 所有cfg都是"FPNPredictor"
