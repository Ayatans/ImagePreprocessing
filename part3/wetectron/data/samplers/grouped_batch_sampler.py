...
class GroupedBatchSampler(BatchSampler):
    ...
    def __init__(self, sampler, group_ids, batch_size, b_size, dataset, class_batch, data_args = None, drop_uneven=False):
        ...
        self.group_ids = torch.as_tensor(group_ids) # 7个0剩下全是1 shape11725 是aspect ratio*aspect grouping
        ...
        self.class_batch = class_batch  # False
        self.b_size = b_size
    ...
    def _prepare_batches(self):
        dataset_size = len(self.group_ids)  # 11725
        # get the sampled indices from the sampler
        # 每张卡用的图的id 随机的 len=2932个*4卡=11728， 实际11725个训图
        # [0-11724]闭区间 这个没什么问题
        ...
        # 开始全-1, 11725 长度，每张卡都是
        order = torch.full((dataset_size,), -1, dtype=torch.int64)
        # 该卡用的图片索引处，排好顺序
        order[sampled_ids] = torch.arange(len(sampled_ids))
        # get a mask with the elements that were sampled len=11725 全是True和False
        mask = order >= 0

        # find the elements that belong to each individual cluster
        # group_ids有7个0剩下全1 该操作是给分组了
        clusters = [(self.group_ids == i) & mask for i in self.groups] # i=0, 1
        ...
        # permuted_clusters是长度为2的list，每项是tensor，tensor里是list，包含ind，两项总长2932，但空的那个不算长度，所以0+2932
        # splits each cluster in batch_size, and merge as a list of tensors
        # splits是通过split方法，将上面本来在一个tensor里的拆分成bs长的tensor的组合，总量不变。
        splits = [c.split(self.batch_size) for c in permuted_clusters]
        # splits分的就不太对，每个里面有2部分，两部分总长度有3卡是2932,1卡是2933，多了一个空的

        merged = tuple(itertools.chain.from_iterable(splits))
        # 4卡bs4时，3张len merged=2932,1张为2933，其merged的第0项是空的。
        # merged是分配给每个卡的，使用的数据的索引，tuple里全是一个整数组成的tensor
