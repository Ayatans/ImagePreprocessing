...
def im_detect_bbox_aug(model, images, device, meta_attentions, targets):
    ...
    boxlists_i = im_detect_bbox(
        model, images, cfg.INPUT.MIN_SIZE_TEST, cfg.INPUT.MAX_SIZE_TEST, device, meta_attentions, targets
    )
    ...
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        ...
        boxlists_scl = im_detect_bbox_scale(
            model, images, scale, max_size, device, meta_attentions, targets
        )
        add_preds_t(boxlists_scl)

        if cfg.TEST.BBOX_AUG.SCALE_H_FLIP:
            boxlists_scl_hf = im_detect_bbox_scale(
                model, images, scale, max_size, device, meta_attentions, targets, hflip=True,
            )
...
def im_detect_bbox(model, images, target_scale, target_max_size, device, meta_attentions, targets):
    """
    ...
    return model(images.to(device),[target.to(device) for target in targets], meta_attentions=meta_attentions)


def im_detect_bbox_hflip(model, images, target_scale, target_max_size, device, meta_attentions, targets):
    """
    ...
    images = [transform(image) for image in images]
    images = to_image_list(images, cfg.DATALOADER.SIZE_DIVISIBILITY)
    boxlists = model(images.to(device),[target.to(device) for target in targets], meta_attentions=meta_attentions)
...
def im_detect_bbox_scale(model, images, target_scale, target_max_size, device, meta_attentions, targets, hflip=False):
    """
    Computes bbox detections at the given scale.
    Returns predictions in the scaled image space.
    """
    if hflip:
        boxlists_scl = im_detect_bbox_hflip(model, images, target_scale, target_max_size, device, meta_attentions, targets)
    else:
        boxlists_scl = im_detect_bbox(model, images, target_scale, target_max_size, device, meta_attentions, targets)
    return boxlists_scl
