...
class DIORDataset_Meta(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # # # 重新构建，模仿PCNN
        img_ids = self.ids[index]   # index：每次bs per GPU个int�?
        data = []
        for cls_id, img_id in img_ids:
            img = cv2.imread(img_id, cv2.IMREAD_COLOR)
            img = img.astype(np.float32, copy=False)
            ...
            mask[:, :] = 0
            mask = torch.from_numpy(mask).unsqueeze(0).unsqueeze(3)
            for obj in target.iter('object'):  # 遍历个某张图内所有的<object>元素。也就是1个标�?
                ...
                for i, pt in enumerate(pts):
                    ...
                    if i % 2 == 0:  # i是偶数，读取到的是xmin，xmax
                        ...
                        cur_pt = int(cur_pt)
                        bndbox.append(cur_pt)  # 相当于也是按pts的顺序加入bndbox�?
            ...
            if xd > yd:
                ...
                xdown = 128 + int(xd / 2)
                mask[:, xup:xdown] = 1
                # 现在image patch �?个旋转patch大小都是256*256
    ...
    def crop(self, image, purpose, size):
        # 裁剪purpose区域内的图片并resize到size，保持bbox宽高比不变，其余部分�?
        ...
        # 不管max是宽还是高，x0 y0都是0，x1 y1都是width height，只是可能抛弃了一两个点因为触�?
