...
seed = sys.argv[1]
baseclass = ('airplane', 'baseballdiamond', "basketballcourt", "bridge", "groundtrackfield",
               "harbor", "ship", "storagetank", 'tenniscourt', "vehicle")
classes=['airplane', 'baseballdiamond', 'tennis court']

few_nums = [3, 5, 10]
DROOT = '/remote-home/yczhang/code/dataset/NWPU-VOC'
root =  DROOT + '/nwpulist' + str(seed) + '/'
rootfile =  DROOT + '/trainval.txt'   # 520ge
...
def get_bbox_fewlist(rootfile, shot):
    ...
    random.seed(3)
    cls_lists = [[] for _ in range(len(baseclass))]   # 20 lists
    cls_counts = [0] * len(baseclass) # 每类计数器
...
def gen_bbox_fewlist():
    ...
    for n in few_nums:
        ...
        for i, clsname in enumerate(baseclass):
            print('   | Processing class: {}'.format(clsname))
            with open(path.join(root, 'box_{}shot_{}_trainval.txt'.format(n, clsname)), 'w') as f:
