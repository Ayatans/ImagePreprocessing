...
def train(cfg, local_rank, distributed, use_tensorboard=False):
    model = build_detection_model(cfg)  # 根据cfg构建Generalized RCNN
    ...
    arguments = {"iteration": 0, "iter_size": cfg.SOLVER.ITER_SIZE}     # -1
    ...
    extra_checkpoint_data = checkpointer.load(cfg.MODEL.WEIGHT)     # backbone weight
...
def run_test(cfg, model, distributed):
    if distributed:
        model = model.module
    iou_types = ("bbox",)
    output_folders = [None] * len(cfg.DATASETS.TEST)    # len=1
    dataset_names = cfg.DATASETS.TEST
    if cfg.OUTPUT_DIR:
        for idx, dataset_name in enumerate(dataset_names):
            output_folder = os.path.join(cfg.OUTPUT_DIR, "inference"+datetime.datetime.now().strftime('%Y-%m-%d'), dataset_name)
            mkdir(output_folder)
...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/dior/dior_contra_db_b8_lr0.01_singless.yaml",
        # default="configs/nwpuv2/nwpuv2.yaml",
        metavar="FILE",
    ...
    genseed=cfg.SEED + get_rank()
    # seed_all_rng(None if cfg.SEED < 0 else cfg.SEED + get_rank())
    seed_all_rng(None if cfg.SEED < 0 else genseed)
    ...
    logger.info("Seed: {}".format(genseed))
