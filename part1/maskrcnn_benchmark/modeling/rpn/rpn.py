...
class RPNHead(nn.Module):
    ...
    def forward(self, x):
        ...
        for feature in x:
            ...
            t = F.relu(self.conv(feature))  # 输入输出维度不变，3*3卷积
            cls_conv = self.cls_conv(t)     # 输出=anchor数*输入/4
            ...
            cls_logits = self.cls_logits(cls_conv).view(-1, self.num_anchors, cls_conv.size(2), cls_conv.size(3))   # 输入=输入/4，输出=1，1*1
